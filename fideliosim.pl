...
my @options = 
    (
     ...
     'p:s',            # Port
     'trace:i',        # Trace
     'bind_address=s', # Address we bind the port
     );
...
my $opt_bind_address = $main::opt_bind_address || '127.0.0.1';
...
else
{
    ...
    foreach (split(/\s*,\s*/, &Radius::Util::format_special($opt_bind_address)))
    {
...
sub usage
{
    print "usage: $0 [-h] [-trace n] [-bind_address addrdess]
